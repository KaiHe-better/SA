...
class My_Train_Framework:
    ...
    def __initialize_stu__(self, stu_model, load_ckpt_path=None, optimizer="adamw", 
        ...
        # self.stu_model, self.stu_optim, self.stu_scheduler = self.tea_model, self.tea_optim, self.tea_scheduler
    ...
    def chosed_pesudo_label(self, total_ae, total_oe, total_sc, gold_res_list, chosed_th):
        chosed_data_index, total_ae, total_oe, total_sc  = self.weight_pesuo(total_ae, total_oe, total_sc, chosed_th)
        ...
        print("added pesuo_data: {}/{}/{} \n".format(len(mix_data_loader), len(self.train_data_loader), \
                                                     len(self.unlabled_data_loader)+len(self.train_data_loader)), file=self.my_print_file)

        return mix_data_loader, (gold_chosed_ae_label, gold_chosed_oe_label, gold_chosed_sc_label)

    def weight_pesuo(self, total_ae, total_oe, total_sc, chosed_th, uncertain_ratio=0.5):
        ...
        chosed_data_index = list( set(ae_chose).intersection(set(sc_chose)).intersection(set(oe_chose))  )
        
        
        add_data_len = int(total_ae.size()[0] *uncertain_ratio)
        ...
        uncertain_chosed_index = torch.sort(uncertain)[1][:add_data_len]
        
        total_chosed_data_index = list(set(chosed_data_index).intersection(set(torch.sort(uncertain_chosed_index)[1].tolist())))
        # total_chosed_data_index = chosed_data_index
        return total_chosed_data_index, ae, oe, sc
        
    def train_stu_gold_data(self, epochs):
    ...
    def train_tea_gold_data(self, epochs):
    ...
    def generate_mix_data_loader(self, chosed_th):
        self.tea_model.eval()
        ...
        mix_data_loader, gold_res = self.chosed_pesudo_label(logits_ae_list, logits_oe_list, logits_sc_list, total_gold_res_list, chosed_th)
      
        return mix_data_loader, gold_res
    
    def weig_train_stu_all_data(self, epochs, mix_data_loader, gold_res):
        self.stu_model.train()
        ...
        for epoch in range(epochs):
            with tqdm(total=len(mix_data_loader), desc="train_stu_all_data", ncols=170) as pbar:  
                for tensor_data_list in mix_data_loader:
                    ...
                    weighted_stu_psuedo_loss, _ = self.my_weighter(copy_stu_mix_dic, mix_my_target)
                    ...
                    try:
                        ...
                        weighted_wig_psuedo_loss, _ = self.my_weighter(copy_stu_gold_dic, mix_my_target)
                    ...
                    weighted_total_loss, mean_weig = self.my_weighter(stu_mix_dic, mix_my_target)
                    if self.if_fp16:
                    ...
                    postfix['mean_weig']= '{0:.4f}'.format(torch.mean(mean_weig)) 
                    pbar.set_postfix(postfix)
                    pbar.update(1)
        ...
        if self.opt.record_weig:
            with open(self.res_path+"/"+"weights.txt", "w") as f:
                for i in mean_weig:
                    f.write(str(i.tolist()[0:100])+"\r")   
                    f.write(str(i.tolist()[100:200])+"\r")   
                    f.write(str(i.tolist()[200:300])+"\r")   
                
        return mix_train_dic_F, stu_mix_dic
    
    def no_weig_train_stu_all_data(self, epochs, mix_data_loader, gold_res):
        self.stu_model.train()
        mix_epoch_input_list, mix_epoch_target_list, mix_epoch_pred_list = [],[],[]
        for epoch in range(epochs):
            with tqdm(total=len(mix_data_loader), desc="train_stu_all_data", ncols=170) as pbar:  
        ...
        mix_train_dic_F = self.get_measure("Train", mix_epoch_input_list, mix_epoch_target_list, mix_epoch_pred_list, record_flag="Pesuo", res_list=gold_res)  
        print("total_loss:{:.4f}, mix_contra_loss：{:.4f}".format(total_loss, mix_contra_loss), file=open(self.res_path+'/training.txt', "a", encoding='utf-8'))    
        return mix_train_dic_F, stu_mix_dic
    
    def train(self, tea_model, stu_model, my_weighter, optim_name, save_ckpt_path, load_ckpt_path, corpus=None,
              if_weight_loss=False, epochs=1, total_epochs=1000,  weight_decay=0, chosed_th=0.5,
              encoder_lr=2e-5, weight_lr=2e-5, classifier_lr=2e-5):
        
        self.tokenizer = tea_model.sentence_encoder.tokenizer
        self.__initialize_tea__(tea_model, load_ckpt_path, optim_name, weight_decay, encoder_lr, classifier_lr, total_epochs*epochs)
        self.__initialize_stu__(stu_model, load_ckpt_path, optim_name, weight_decay, encoder_lr, classifier_lr, total_epochs*epochs)
        if if_weight_loss:
            self.__initialize_weighter__(my_weighter, load_ckpt_path, optim_name, weight_decay, weight_lr, total_epochs*epochs)
        ...
        for Total in range(total_epochs):
            ...
            _ = self.train_tea_gold_data(epochs)
            pesuo_data_loader_with_label, gold_res = self.generate_mix_data_loader(chosed_th)
            
            _ = self.train_stu_gold_data(epochs)
            
            if if_weight_loss:
                mix_train_dic_F, stu_mix_dic = self.weig_train_stu_all_data(epochs, pesuo_data_loader_with_label, gold_res) 
            else:
                 mix_train_dic_F, stu_mix_dic = self.no_weig_train_stu_all_data(epochs, pesuo_data_loader_with_label, gold_res) 
            ...
            if stu_val_dic_F["ABSA_F"] > best_f_absa:
                ...
                # torch.save({'state_dict': stu_model.state_dict()}, save_ckpt_path)
