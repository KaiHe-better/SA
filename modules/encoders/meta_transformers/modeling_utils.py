...
def get_parameter_device(parameter: Union[MetaModule, GenerationMixin, "ModuleUtilsMixin"]):
    try:
        return next(parameter.parameters()).device
    except StopIteration:
        # For nn.DataParallel compatibility in PyTorch 1.5

        def find_tensor_attributes(module: MetaModule) -> List[Tuple[str, Tensor]]:
...
def get_parameter_dtype(parameter: Union[MetaModule, GenerationMixin, "ModuleUtilsMixin"]):
    try:
...
class ModuleUtilsMixin:
    """
    A few utilities for :obj:`torch.MetaModules`, to be used as a mixin.
    """
...
class PreTrainedModel(MetaModule, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    @property
    def base_model(self) -> MetaModule:
        """
        :obj:`torch.MetaModule`: The main body of the model.
        """
        return getattr(self, self.base_model_prefix, self)

    def get_input_embeddings(self) -> MetaModule:
        """
        Returns the model's input embeddings.

        Returns:
            :obj:`MetaModule`: A torch module mapping vocabulary to hidden states.
        """
    ...
    def set_input_embeddings(self, value: MetaModule):
        """
        Set model's input embeddings.

        Args:
            value (:obj:`MetaModule`): A module mapping vocabulary to hidden states.
        """
        base_model = getattr(self, self.base_model_prefix, self)
        if base_model is not self:
    ...
    def get_output_embeddings(self) -> MetaModule:
        """
        Returns the model's output embeddings.

        Returns:
            :obj:`MetaModule`: A torch module mapping hidden states to vocabulary.
        """
    ...
    @staticmethod
    def _tie_encoder_decoder_weights(encoder: MetaModule, decoder: MetaModule, base_model_prefix: str):
        ...
        def tie_encoder_to_decoder_recursively(
            ...
            assert isinstance(decoder_pointer, MetaModule) and isinstance(
                encoder_pointer, MetaModule
            ), f"{decoder_pointer} and {encoder_pointer} have to be of type MetaModule"
            ...
            if len(decoder_modules) > 0:
                ...
                for name, module in decoder_modules.items():
                    if name.isdigit():
                        ...
                        raise ValueError(
                            "Max depth of recursive function `tie_encoder_to_decoder` reached. It seems that there is a circular dependency between two or more `MetaModules` of your model."
                        )
                    else:
    ...
    def _get_resized_lm_head(
        self, old_lm_head: MetaModule, new_num_tokens: Optional[int] = None, transposed: Optional[bool] = False
    ) -> MetaModule:
        """
        ...
        Args:
            old_lm_head (:obj:`torch.MetaModule`):
            ...
            new_num_tokens (:obj:`int`, `optional`):
                ...
                Increasing the size will add newly initialized vectors at the end. Reducing the size will remove
                vectors from the end. If not provided or :obj:`None`, just returns a pointer to the input tokens
                :obj:`torch.MetaModule`` module of the model without doing anything.
        ...
        Return:
            :obj:`torch.MetaModule`: Pointer to the resized Linear Module or the old Linear Module if
            :obj:`new_num_tokens` is :obj:`None`
        """
        if new_num_tokens is None:
            return old_lm_head
        ...
        if not isinstance(old_lm_head, MetaModule):
            raise TypeError(
                f"Old language model head is of type {type(old_lm_head)}, which is not an instance of {MetaModule}. "
                f"You should either use a different resize function or make sure that `old_lm_head` are an instance of {MetaModule}."
            )

        # Build new lm head
        new_lm_head_shape = (old_lm_head_dim, new_num_tokens) if not transposed else (new_num_tokens, old_lm_head_dim)
        has_new_lm_head_bias = old_lm_head.bias is not None
        new_lm_head = MetaModule(*new_lm_head_shape, bias=has_new_lm_head_bias)
    ...
    def _load_state_dict_into_model(
        ...
        def load(module: MetaModule, prefix=""):
...
class PoolerStartLogits(MetaModule):
    """
    ...
    def __init__(self, config: PretrainedConfig):
        super().__init__()
        self.dense = MetaModule(config.hidden_size, 1)
...
class PoolerEndLogits(MetaModule):
    """
    ...
    def __init__(self, config: PretrainedConfig):
        super().__init__()
        self.dense_0 = MetaModule(config.hidden_size * 2, config.hidden_size)
        self.activation = nn.Tanh()
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dense_1 = MetaModule(config.hidden_size, 1)
...
class PoolerAnswerClass(MetaModule):
    """
    ...
    def __init__(self, config):
        ...
        self.dense_0 = MetaModule(config.hidden_size * 2, config.hidden_size)
        self.activation = nn.Tanh()
        self.dense_1 = MetaModule(config.hidden_size, 1, bias=False)
...
class SequenceSummary(MetaModule):
    r"""
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        if hasattr(config, "summary_use_proj") and config.summary_use_proj:
            ...
            self.summary = MetaModule(config.hidden_size, num_classes)
...
def unwrap_model(model: MetaModule) -> MetaModule:
    """
    Recursively unwraps a model from potential containers (as used in distributed training).

    Args:
        model (:obj:`torch.MetaModule`): The model to unwrap.
    """
...
def prune_linear_layer(layer: MetaModule, index: torch.LongTensor, dim: int = 0) -> MetaModule:
    """
    ...
    Args:
        layer (:obj:`torch.MetaModule`): The layer to prune.
        index (:obj:`torch.LongTensor`): The indices to keep in the layer.
        dim (:obj:`int`, `optional`, defaults to 0): The dimension on which to keep the indices.

    Returns:
        :obj:`torch.MetaModule`: The pruned layer as a new layer with :obj:`requires_grad=True`.
    """
    ...
    new_layer = MetaModule(new_size[1], new_size[0], bias=layer.bias is not None).to(layer.weight.device)
...
def prune_layer(
    layer: Union[MetaModule, Conv1D], index: torch.LongTensor, dim: Optional[int] = None
) -> Union[MetaModule, Conv1D]:
    """
    ...
    Args:
        layer (:obj:`Union[torch.MetaModule, Conv1D]`): The layer to prune.
        index (:obj:`torch.LongTensor`): The indices to keep in the layer.
        dim (:obj:`int`, `optional`): The dimension on which to keep the indices.

    Returns:
        :obj:`torch.MetaModule` or :class:`~transformers.modeling_utils.Conv1D`: The pruned layer as a new layer with
        :obj:`requires_grad=True`.
    """
    if isinstance(layer, MetaModule):
...
def apply_chunking_to_forward(
    forward_fn: Callable[..., torch.Tensor], chunk_size: int, chunk_dim: int, *input_tensors, params=None
) -> torch.Tensor:
    """
    ...
    return forward_fn(*input_tensors, params=params)
