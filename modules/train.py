...
parser.add_argument('--batch_size', default=52, type=int, help='number of example per batch :  52')
...
parser.add_argument('--record_weig', action='store_true', default=True,  help='weight_loss')
parser.add_argument('--my_min', default=1, type=float, help='my_min')
parser.add_argument('--my_max', default=3, type=float, help='my_max')

parser.add_argument('--if_fp16', action='store_true', default=False,  help='if_fp16')
...
parser.add_argument('--chosed_th', default=0.9, type=float, help='augment_num')
...
def get_model():
    my_weighter = My_weighter(opt.max_len, opt.my_min, opt.my_max)
    
    # tea_sentence_encoder = BERT_encoder("./pretrain/bert-large-uncased")
    # tea_model = My_model(tea_sentence_encoder, opt.if_weight_loss, opt.max_len, opt.my_min, opt.my_max)
    
    # stu_sentence_encoder = BERT_encoder("./pretrain/bert-large-uncased")
    # stu_model = My_model(stu_sentence_encoder, opt.if_weight_loss, opt.max_len, opt.my_min, opt.my_max)
    
    
    sentence_encoder = BERT_encoder("./pretrain/bert-large-uncased")
    tea_model = My_model(sentence_encoder, opt.if_weight_loss, opt.max_len, opt.my_min, opt.my_max)
    stu_model = My_model(sentence_encoder, opt.if_weight_loss, opt.max_len, opt.my_min, opt.my_max)
...
def main(train_index, train_ID=None):
    ...
    data_loader_list, train_dataset = get_loader(opt.corpus, opt.data_ratio, opt.add_other_corpus, opt.batch_size, opt.max_len, tea_model.sentence_encoder.tokenizer)
    ...
    if not opt.only_test:
        best_epoch, best_dic_F = my_train_framework.train(tea_model, stu_model, my_weighter, opt.optim, opt.save_ckpt_path, opt.load_ckpt_path, 
                                epochs=opt.epochs, total_epochs=opt.total_epochs, corpus=opt.corpus, if_weight_loss=opt.if_weight_loss, 
                                weight_decay=opt.weight_decay, encoder_lr=opt.encoder_lr, weight_lr=opt.weight_lr, classifier_lr=opt.classifier_lr, chosed_th=opt.chosed_th)
