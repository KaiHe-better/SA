...
def get_loader(corpus, data_ratio, add_other_corpus, batch_size, max_len, tokenizer, num_workers=8):
    ...
    for fname in train_dev_test_list:
        ...
        if "train" in fname:
            is_testing = False
            need_sent_data, need_ae_data, need_oe_data, need_sc_data = [],[],[],[] 
            remain_sent_data, remain_ae_data, remain_oe_data, remain_sc_data = [],[],[],[] 
            for i in all_index:
                if i in need_index_list:
                    need_sent_data.append(sent_data[i])
                    need_ae_data.append(ae_data[i])
                    need_oe_data.append(oe_data[i])
                    need_sc_data.append(sc_data[i])
                else:
                    remain_sent_data.append(sent_data[i])
                    remain_ae_data.append(ae_data[i])
                    remain_oe_data.append(oe_data[i])
                    remain_sc_data.append(sc_data[i])
        else:
            is_testing = True
    ...
    if add_other_corpus:
        ...
        for fname in pesu_train_dev_test_list:
            remain_sent_data.extend(open(fname + r'sentence.txt', 'r', encoding='utf-8').readlines())
            remain_ae_data.extend(open(fname + r'target.txt', 'r', encoding='utf-8').readlines())
            remain_oe_data.extend(open(fname + r'opinion.txt', 'r', encoding='utf-8').readlines())
            remain_sc_data.extend(open(fname + r'target_polarity.txt', 'r', encoding='utf-8').readlines())
