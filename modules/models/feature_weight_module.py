...
# class Feature_weight_module(torch.nn.Module):
#     def __init__(self, hidden_size):
#         super().__init__()
#         self.hidden_size = hidden_size
#         self.n_layers = 1

#         self.encode_layers = nn.ModuleList([Transformer_Layer(d_model=hidden_size) for _ in range(self.n_layers)])

#         self.mlp_head = nn.Sequential(nn.Linear(self.hidden_size, self.hidden_size),
#                                       nn.ReLU(),
#                                       nn.Linear(self.hidden_size, self.hidden_size))

#     def forward(self, input_embeds, att_mask=None):
#         # att_mask = torch.ones([input_embeds.size(0), input_embeds.size(1), input_embeds.size(1)]).bool().to(input_embeds.device)
#         for layer in self.encode_layers:
#             input_embeds = layer(input_embeds, att_mask)[0]

#         weight_logits = self.mlp_head(input_embeds).squeeze()
#         return weight_logits



class Feature_weight_module(torch.nn.Module):
    def __init__(self, hidden_size, out_dim=None):
        super().__init__()
        self.hidden_size = hidden_size
        if out_dim is None:
    ...
    # weight = torch.cumsum(F.softmax(weight_logits, -1), dim=-1)
